...
function tracker(endpoint, namespace, appId, encodeBase64) {
	...
	function sendPayload(payload) {
		payload['eid'] = uuid.v4();
		payload['dtm'] = payload['dtm'] || new Date().getTime();
		console.log(payload);
		request({url: endpoint, qs: payload});
	}
	...
	function trackEcommerceTransaction(orderId, affiliation, totalValue, tax, shipping, city, state, country, currency, items, context, tstamp) {
