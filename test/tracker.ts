...
function checkPayload(payloadDict: PayloadDictionary, expected: PayloadDictionary, t: ExecutionContext<unknown>): void {
...
for (const method of testMethods) {
  ...
  test.cb(
    ...
    (t) => {
      ...
      const track = tracker(e, 'cf', 'cfe35', false);
      track.trackEcommerceTransactionWithItems(
        'order-7',
        'affiliate',
        '15',
        '5',
  ...
  test.cb(method + ' method: trackUnstructEvent should send a structured event', (t) => {
    ...
    const track = tracker(e, 'cf', 'cfe35', false);
    track.trackUnstructEvent(inputJson, context);
  });
  ...
  test.cb(method + ' method: trackScreenView should send a screen view event', (t) => {
    ...
    const track = tracker(e, 'cf', 'cfe35', false);
    track.trackScreenView('title screen', '12345', context);
  });
  ...
  test.cb(method + ' method: setter methods should set user attributes', (t) => {
    ...
    track.trackPageView('http://www.example.com', 'example page', 'google', context, 1000000000000);
  });
